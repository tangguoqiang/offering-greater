...
//	public static void cutImage(String src, String dest, String suff,  int x,
//			int y, int w, int h) throws IOException {
//		if (Utils.isEmpty(suff))
//			suff = "jpg";
//		Iterator iterator = ImageIO.getImageReadersByFormatName(suff);
//		ImageReader reader = (ImageReader) iterator.next();
//		InputStream in = new FileInputStream(src);
//		ImageInputStream iis = ImageIO.createImageInputStream(in);
//		reader.setInput(iis, true);
...
//		param.setSourceRegion(rect);
//		BufferedImage bi = reader.read(0, param);
//		ImageIO.write(bi, suff, new File(dest));
//	}
     ...
     * @param h  目标切片 高度 
     */  
    public static void cutImage(String imagePath,String dest,String suff, ImageCoord coord){  
        try {  
            Image img;  
            ...
            //若原图大小大于切片大小，则进行切割  
            if (srcWidth >= coord.getW() && srcHeight >= coord.getH()) {  
                Image image = bi.getScaledInstance(srcWidth, srcHeight,Image.SCALE_DEFAULT);  
                  
                int x1 = coord.getX()*srcWidth/300;  
                int y1 = coord.getY()*srcHeight/300;  
                int w1 = coord.getW()*srcWidth/300;  
                int h1 = coord.getH()*srcHeight/300;  
                ...
                g.dispose();  
                // 输出为文件  
                ImageIO.write(tag, suff, new File(dest));  
            }  
        } catch (IOException e) {  
            e.printStackTrace();  
        }  

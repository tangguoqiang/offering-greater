...
public class UserServiceImpl implements UserService{
	...
	public User getUserInfoByPhone(String phone,String password){
		String sql = " select id,name,nickname,type,token,password,url,rc_token from USER_INFO WHERE phone=? ";
		if(password != null)
			sql += "AND password=?";
	...
	public User getUserInfoByNickName(String nickname)
	{
		...
		sql.append("select id,nickname ")
		   .append("from USER_INFO ")
		   .append("WHERE nickname=?");
		ParamInfo paramInfo = new ParamInfo();
		paramInfo.setTypeAndData(Types.VARCHAR, nickname);
		return userDao.getRecord(sql.toString(),paramInfo,User.class);
	}
...
//		}
	...
	public long insertUser(User user){
		return userDao.insertRecord(user, "USER_INFO");
	}
